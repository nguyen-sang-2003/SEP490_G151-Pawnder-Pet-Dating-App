    ...
    public class PetImageAnalysisService : IPetImageAnalysisService
    {
        ...
        public async Task<PetImageAnalysisResponse> AnalyzeImageAsync(IFormFile image)
        {
            try
            {
                ...
                // Build prompt for AI (includes cat validation)
                var prompt = BuildAnalysisPrompt(attributes);

                // Call AI API (Google Gemini) - Single call for both cat check and analysis
                var analysisResult = await CallGeminiVisionAPI(base64Image, prompt, image.ContentType);

                // Check if it's not a cat (special response)
                if (analysisResult != null && analysisResult.Count == 1 && 
                    analysisResult[0].AttributeName == "__NOT_A_CAT__")
                {
                    var animalType = analysisResult[0].OptionName;
                    var message = string.IsNullOrEmpty(animalType) || animalType.ToLower() == "null" || animalType.ToLower() == "không xác định"
        ...
        private string BuildAnalysisPrompt(List<Models.Attribute> attributes)
        {
            ...
            // Thêm yêu cầu kiểm tra mèo trước
            promptBuilder.AppendLine("BƯỚC 1: Kiểm tra xem ảnh có chứa con MÈO (cat) hay không.");
            promptBuilder.AppendLine();
            promptBuilder.AppendLine("NẾU ảnh KHÔNG chứa con mèo (ví dụ: chó, chim, người, đồ vật, v.v.), hãy trả về:");
            promptBuilder.AppendLine("[");
            promptBuilder.AppendLine("  {");
            promptBuilder.AppendLine("    \"attributeName\": \"__NOT_A_CAT__\",");
            promptBuilder.AppendLine("    \"optionName\": \"<tên loại động vật hoặc đối tượng trong ảnh>\"");
            promptBuilder.AppendLine("  }");
            promptBuilder.AppendLine("]");
            promptBuilder.AppendLine();
            promptBuilder.AppendLine("NẾU ảnh CÓ chứa con mèo, tiếp tục BƯỚC 2.");
            promptBuilder.AppendLine();
            promptBuilder.AppendLine("BƯỚC 2: Phân tích ảnh mèo và trả về thông tin về các đặc điểm sau dưới dạng JSON:");
            promptBuilder.AppendLine();
            ...
            promptBuilder.AppendLine("- Nếu KHÔNG phải mèo, chỉ trả về __NOT_A_CAT__ như BƯỚC 1.");
